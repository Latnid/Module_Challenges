...
def recommend_portfolio(intent_request):
    ...
    if risk_level == 'NONE':
        rc_portfolio = "100% bonds (AGG), 0% equities (SPY)"
    elif risk_level == 'LOW':
        rc_portfolio = "60% bonds (AGG), 40% equities (SPY)"
    elif risk_level == 'MEDIUM':
        rc_portfolio = "40% bonds (AGG), 60% equities (SPY)"
    elif risk_level == 'HIGH':
        rc_portfolio = "20% bonds (AGG), 80% equities (SPY)"
    ...
    return close(
        ...
        "Fulfilled",
        {
            ...
            The recommend portfolio for you is {}.
            """.format(
                rc_portfolio
            ),
        },
...
def validate_data(first_name, age, investment_amount, risk_level, intent_request):
    ...
    """      
 # Validate the age greater than zero and less than 65.  
    if age is not None:
 ...
 # Validate the investment amount is equal to or greater than 5000.
    if investment_amount is not None:
        ...
        if investment_amount <= 5000:
            return build_validation_result(
                    False,
                    "investmentAmount",
                    "The investment amount should more than $5000, "
                    "please provide a correct invest amount.",)

# A True results is returned if age and amount are valid
    return build_validation_result(True, None, None)

# Helper function for data to float
def parse_float(n):
    """
